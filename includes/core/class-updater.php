...
final class Plugin_UI_Suite_Updater {
  ...
  public static function render_updates_page() {
    if (!current_user_can('update_plugins')) return;
    $release = self::latest_release(!empty($_GET['checked']));
    $latest = $release['version'] ?? __('Unknown','plugin-ui-suite');
    $is_current = !empty($release['version']) ? version_compare($release['version'], PLUGIN_SUITE_VERSION, '<=') : null;
    echo '<div class="wrap plugin-suite-updates"><h1>' . esc_html__('Rescue Plugin Suite Updates', 'plugin-ui-suite') . '</h1>';
    echo '<style>.plugin-suite-update-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:16px;margin:18px 0}.plugin-suite-update-card{background:#fff;border:1px solid #dcdcde;border-radius:14px;padding:16px}.plugin-suite-update-card strong{display:block;font-size:1.4rem;margin-top:6px}.plugin-suite-release-notes{background:#fff;border:1px solid #dcdcde;border-radius:14px;padding:18px;max-width:980px}.plugin-suite-update-actions{display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin:16px 0}</style>';
    if (!empty($_GET['checked'])) echo '<div class="notice notice-success"><p>' . esc_html__('GitHub was checked just now. This manual check bypassed the scheduled update cache.', 'plugin-ui-suite') . '</p></div>';
    if (!empty($_GET['saved'])) echo '<div class="notice notice-success"><p>' . esc_html__('Automatic update preference saved.', 'plugin-ui-suite') . '</p></div>';
    echo '<p class="description">' . esc_html__('Use this page to check for a new release, read what changed, and decide whether WordPress may update the suite automatically.', 'plugin-ui-suite') . '</p>';
    echo '<div class="plugin-suite-update-grid">';
    foreach ([__('Installed version','plugin-ui-suite')=>PLUGIN_SUITE_VERSION, __('Latest version','plugin-ui-suite')=>$latest, __('Repository','plugin-ui-suite')=>self::REPO, __('Last checked','plugin-ui-suite')=>get_option(self::LAST_CHECK_KEY, __('Never','plugin-ui-suite')), __('Automatic updates','plugin-ui-suite')=>(get_option(self::AUTO_UPDATES_KEY,0)?__('Enabled','plugin-ui-suite'):__('Disabled','plugin-ui-suite')), __('Status','plugin-ui-suite')=>($is_current === null ? __('Unable to confirm','plugin-ui-suite') : ($is_current ? __('Up to date','plugin-ui-suite') : __('Update available','plugin-ui-suite')))] as $label=>$value) echo '<div class="plugin-suite-update-card"><span>' . esc_html($label) . '</span><strong>' . esc_html($value) . '</strong></div>';
    echo '</div>';
    echo '<form class="plugin-suite-update-actions" method="post">'; wp_nonce_field('plugin_suite_updates'); echo '<button class="button button-primary" name="plugin_suite_check_updates" value="1">' . esc_html__('Check for updates now', 'plugin-ui-suite') . '</button> <a class="button" href="' . esc_url(PLUGIN_SUITE_URL . 'changelog.md') . '">' . esc_html__('View changelog', 'plugin-ui-suite') . '</a> <a class="button" target="_blank" rel="noopener" href="' . esc_url('https://github.com/' . self::REPO . '/releases') . '">' . esc_html__('View releases', 'plugin-ui-suite') . '</a></form>';
    ...
    echo '<h2>' . esc_html__('Release notes', 'plugin-ui-suite') . '</h2><div class="plugin-suite-release-notes">';
    if (!empty($release['body'])) echo wp_kses_post(wpautop($release['body'])); else echo '<p>' . esc_html__('No release notes were returned by GitHub. Try checking again later or open GitHub releases.', 'plugin-ui-suite') . '</p>';
    echo '</div></div>';
  }}
