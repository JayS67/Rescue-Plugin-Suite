...
final class Plugin_Payments_Module {
  ...
  public static function register_registry_metadata() {
    if (!class_exists('Plugin_UI_Suite_Registry')) return;
    Plugin_UI_Suite_Registry::register_module('payments', ['name'=>'Payments','description'=>'Donation providers, campaigns, Gift Aid, fee recovery, transactions and reports.','icon'=>'money-alt','flags'=>['installed'=>true,'enabled'=>true,'hidden'=>false,'experimental'=>false,'beta'=>false,'deprecated'=>false,'future_premium'=>false]]);
    foreach (self::gateway_manager()->all() as $id=>$provider) {
      Plugin_UI_Suite_Registry::register('integrations', 'payment_'.$id, ['module'=>'payments','type'=>'payment_provider','name'=>$provider->get_name(),'configuration_fields'=>$provider->get_configuration_fields(),'capabilities'=>$provider->get_capabilities(),'supported_features'=>array_keys(array_filter($provider->get_capabilities())),'conditional_settings'=>['active_provider'=>$id]]);
    }
    $defaults = self::defaults();
    $settings = [
      'active_provider'=>['label'=>'Active payment provider','field_type'=>'select','options'=>array_map(function($p){ return $p->get_name(); }, self::gateway_manager()->all()),'page'=>'providers','section'=>'providers','search_keywords'=>['Stripe','PayPal','Square','GoCardless','SumUp','provider']],
      'general_enabled'=>['label'=>'Enable payments','field_type'=>'checkbox','page'=>'general','section'=>'general','field_id'=>'enabled','default'=>$defaults['general']['enabled'],'description'=>'Allow donation checkout from public widgets.'],
      'general_currency'=>['label'=>'Currency','field_type'=>'text','page'=>'general','section'=>'general','field_id'=>'currency','default'=>$defaults['general']['currency'],'search_keywords'=>['currency','GBP','USD','EUR']],
      'general_title'=>['label'=>'Widget title','field_type'=>'text','page'=>'widget','section'=>'content','field_id'=>'title','default'=>$defaults['general']['title'],'search_keywords'=>['widget','donation']],
      'general_subtitle'=>['label'=>'Widget subtitle','field_type'=>'text','page'=>'widget','section'=>'content','field_id'=>'subtitle','default'=>$defaults['general']['subtitle'],'search_keywords'=>['widget','donation']],
      'gift_aid_enabled'=>['label'=>'Enable Gift Aid','field_type'=>'checkbox','page'=>'gift-aid','section'=>'gift_aid','field_id'=>'enabled','default'=>$defaults['gift_aid']['enabled'],'search_keywords'=>['Gift Aid']],
      'gift_aid_wording'=>['label'=>'Gift Aid wording','field_type'=>'text','page'=>'gift-aid','section'=>'gift_aid','field_id'=>'wording','default'=>$defaults['gift_aid']['wording'],'search_keywords'=>['Gift Aid']],
      'fee_recovery_enabled'=>['label'=>'Enable fee recovery','field_type'=>'checkbox','page'=>'fees','section'=>'fee_recovery','field_id'=>'enabled','default'=>$defaults['fee_recovery']['enabled'],'search_keywords'=>['fees','cover fees']],
      'fee_recovery_payer'=>['label'=>'Who covers fees?','field_type'=>'select','page'=>'fees','section'=>'fee_recovery','field_id'=>'payer','default'=>$defaults['fee_recovery']['payer'],'options'=>['rescue'=>'Rescue absorbs fees','donor'=>'Donor covers fees'],'search_keywords'=>['fees','donor covers fees']],
      'appearance_primary'=>['label'=>'Primary colour','field_type'=>'color','page'=>'appearance','section'=>'appearance','field_id'=>'primary','default'=>$defaults['appearance']['primary'],'search_keywords'=>['appearance','colour']],
    ];
    foreach ($settings as $id=>$meta) {
      $module_section = explode('_', $id, 2)[0];
      Plugin_UI_Suite_Registry::register_setting('payments_'.$id, $meta + ['module'=>'payments','field_id'=>$meta['field_id'] ?? $id,'default'=>$meta['default'] ?? '']);
    }
    foreach (['dashboard'=>'Dashboard','general'=>'General','providers'=>'Providers','campaigns'=>'Campaigns','widget'=>'Donation Widget','gift-aid'=>'Gift Aid','fees'=>'Cover Fees','appearance'=>'Appearance','embed'=>'Embed','transactions'=>'Transactions','reports'=>'Reports','diagnostics'=>'Diagnostics','help'=>'Help'] as $i=>$label) Plugin_UI_Suite_Registry::register_navigation('payments_'.$i, ['context'=>'payments','module'=>'payments','slug'=>$i,'label'=>$label,'order'=>count(Plugin_UI_Suite_Registry::navigation_items('payments'))]);
  }

  public static function init() { self::register_registry_metadata(); add_action('admin_init',[__CLASS__,'register_settings']); add_action('rest_api_init',[__CLASS__,'register_rest_routes']); add_action('admin_enqueue_scripts',[__CLASS__,'enqueue_admin_assets']); add_action('wp_ajax_plugin_payments_checkout',[__CLASS__,'handle_checkout']); add_action('wp_ajax_nopriv_plugin_payments_checkout',[__CLASS__,'handle_checkout']); add_action('admin_post_plugin_payments_export_gift_aid',[__CLASS__,'export_gift_aid']); add_shortcode('donation_widget',[__CLASS__,'render_shortcode']); add_action('init',[__CLASS__,'register_block']); add_action('wp_enqueue_scripts',[__CLASS__,'enqueue_frontend_assets']); }
  public static function gateway_manager() { static $m; if (!$m) $m = new Plugin_Payment_Gateway_Manager(); return $m; }
  public static function defaults() { return ['active_provider'=>'stripe','provider_settings'=>[],'general'=>['enabled'=>1,'test_mode'=>1,'live_mode'=>0,'default_type'=>'one_off','currency'=>'GBP','success_url'=>home_url('/thank-you/'),'cancel_url'=>home_url('/'),'button_text'=>'Continue','thank_you_message'=>'Thank you for your support.','enable_campaigns'=>1,'title'=>'Support our work','subtitle'=>'Choose an amount that works for you','intro_text'=>'','learn_more_url'=>''],'one_off'=>['enabled'=>1,'allow_custom'=>1,'min'=>1,'max'=>10000,'default'=>5,'presets'=>[['amount'=>'3','description'=>'Makes a small contribution'],['amount'=>'5','description'=>'Supports essential work'],['amount'=>'10','description'=>'Helps fund ongoing services'],['amount'=>'17.50','description'=>'Creates meaningful impact']]],'recurring'=>['enabled'=>1,'allow_custom'=>1,'min'=>1,'max'=>1000,'default'=>5,'presets'=>[['amount'=>'3','description'=>'Provides steady monthly support'],['amount'=>'5','description'=>'Funds reliable monthly help'],['amount'=>'10','description'=>'Sustains ongoing work'],['amount'=>'17.50','description'=>'Creates lasting impact']]],'campaigns'=>[],'gift_aid'=>['enabled'=>0,'wording'=>'Add Gift Aid to this donation.','declaration'=>'I confirm I am a UK taxpayer and want this charity to reclaim Gift Aid on my donation.','required'=>0,'store_declaration'=>1],'fee_recovery'=>['enabled'=>0,'payer'=>'rescue','override_defaults'=>0,'percentage'=>'','fixed'=>'','wording'=>'I would like to cover payment processing fees.'],'appearance'=>['primary'=>'#401268','background'=>'#ffffff','text'=>'#1f2937','font_family'=>'','button_style'=>'rounded','spacing'=>16,'radius'=>16,'shadow'=>1,'mode'=>'light','show_logo'=>0]]; }
  ...
  public static function sanitize_settings($input) {
    ...
    $clean['fee_recovery']['payer']=in_array(($fr['payer']??'rescue'), ['rescue','donor'], true) ? $fr['payer'] : 'rescue';
    $clean['fee_recovery']['override_defaults']=!empty($fr['override_defaults'])?1:0;
    $clean['fee_recovery']['percentage']=$clean['fee_recovery']['override_defaults'] ? (string)max(0,(float)($fr['percentage']??0)) : '';
    $clean['fee_recovery']['fixed']=$clean['fee_recovery']['override_defaults'] ? (string)max(0,(float)($fr['fixed']??0)) : '';
  ...
  private static function campaign_metrics($slug, $currency='GBP') {
    $slug = sanitize_key($slug);
    $rows = self::dashboard_rows(['campaign'=>$slug,'currency'=>strtoupper($currency)]);
    $successful = ['succeeded','paid','captured','completed','created','declared'];
    $metrics = ['raised'=>0.0,'donor_count'=>0,'recurring_monthly'=>0.0,'recent'=>[]];
    $donors = [];
    foreach ($rows as $row) {
      if (!in_array((string)($row['status'] ?? ''), $successful, true)) continue;
      $amount = (float)($row['amount'] ?? 0);
      if (($row['type'] ?? '') === 'recurring') $metrics['recurring_monthly'] += $amount;
      else $metrics['raised'] += $amount;
      if (!empty($row['id'])) $donors[$row['id']] = true;
      $metrics['recent'][] = $row;
    }
    $metrics['donor_count'] = count($donors);
    usort($metrics['recent'], function($a,$b){ return strcmp((string)($b['date'] ?? ''), (string)($a['date'] ?? '')); });
    $metrics['recent'] = array_slice($metrics['recent'], 0, 5);
    return $metrics;
  }
  private static function campaign_with_metrics(array $campaign, $currency='GBP') {
    $metrics = self::campaign_metrics($campaign['slug'] ?? '', $currency);
    $goal = max(0, (float)($campaign['goal'] ?? 0));
    $campaign['amount_raised'] = $metrics['raised'];
    $campaign['progress'] = $metrics['raised'];
    $campaign['_metrics'] = $metrics + ['goal'=>$goal,'remaining'=>max(0, $goal - $metrics['raised']),'percentage'=>$goal > 0 ? min(100, ($metrics['raised'] / $goal) * 100) : 0];
    return $campaign;
  }
  ...
  private static function apply_campaign($settings, $campaign_slug) {
    ...
    $campaign = self::campaign_with_metrics($settings['campaigns'][$slug], $settings['general']['currency'] ?? 'GBP');
  ...
  private static function provider_fee_profiles() {
    return apply_filters('plugin_payments_provider_fee_profiles', [
      'stripe'=>['name'=>'Stripe','rate'=>0.015,'fixed'=>0.20,'note'=>'UK charity card estimate; adjust through provider config if your account differs.'],
      'paypal'=>['name'=>'PayPal','rate'=>0.029,'fixed'=>0.30,'note'=>'Standard online donation estimate.'],
      'square'=>['name'=>'Square','rate'=>0.0175,'fixed'=>0.00,'note'=>'UK online card estimate.'],
      'gocardless'=>['name'=>'GoCardless','rate'=>0.01,'fixed'=>0.20,'cap'=>4.00,'note'=>'Direct Debit estimate with cap.'],
      'sumup'=>['name'=>'SumUp','rate'=>0.025,'fixed'=>0.00,'note'=>'Online payment link estimate.'],
    ]);
  }
  private static function fee_profile($provider_id, $currency='GBP') {
    $currency = strtoupper($currency);
    $profiles = self::provider_fee_profiles();
    $profile = $profiles[$provider_id] ?? ['rate'=>0,'fixed'=>0,'note'=>'No processing fee profile configured.'];
    if (in_array($currency, ['JPY','KRW','VND'], true)) $profile['fixed'] = 0;
    $settings = self::settings();
    if (!empty($settings['fee_recovery']['override_defaults'])) {
      $profile['rate'] = ((float)($settings['fee_recovery']['percentage'] ?? 0)) / 100;
      $profile['fixed'] = (float)($settings['fee_recovery']['fixed'] ?? 0);
      $profile['note'] = __('Advanced manual override is active.', 'plugin-ui-suite');
    }
  ...
  public static function handle_checkout() {
    ...
    $donor_covers = (!empty($s['fee_recovery']['enabled']) && ($s['fee_recovery']['payer'] ?? 'rescue') === 'donor') || !empty($_POST['fee_recovery']);
    $checkout_amount = (!empty($caps['fee_recovery']) && $donor_covers) ? self::amount_with_fee_recovery($amount, $s['active_provider'], $s['general']['currency']) : $amount;
    $request = ['amount'=>$checkout_amount,'base_amount'=>$amount,'type'=>$type,'payment_type'=>$definition,'currency'=>$s['general']['currency'],'success_url'=>$s['general']['success_url'],'cancel_url'=>$s['general']['cancel_url'],'campaign'=>$campaign,'fee_recovery'=>$donor_covers,'gift_aid'=>!empty($_POST['gift_aid'])];
    /**
...
<section id="<?php echo esc_attr($widget_id); ?>" class="plugin-payments-widget plugin-payments-<?php echo esc_attr($s['appearance']['mode']); ?>" style="--ssp-primary:<?php echo esc_attr($s['appearance']['primary']); ?>;--ssp-on-primary:<?php echo esc_attr(self::readable_text_color($s['appearance']['primary'])); ?>;--ssp-bg:<?php echo esc_attr($s['appearance']['background']); ?>;--ssp-text:<?php echo esc_attr($s['appearance']['text']); ?>;--ssp-radius:<?php echo (int)$s['appearance']['radius']; ?>px" data-currency="<?php echo esc_attr($s['general']['currency']); ?>" data-default-type="<?php echo esc_attr($default); ?>" data-provider="<?php echo esc_attr($p?$p->get_id():''); ?>" data-campaign="<?php echo esc_attr($s['active_campaign']['slug'] ?? ''); ?>" data-fee-rate="<?php echo esc_attr(self::fee_profile($s['active_provider'],$s['general']['currency'])['rate'] ?? 0); ?>" data-fee-fixed="<?php echo esc_attr(self::fee_profile($s['active_provider'],$s['general']['currency'])['fixed'] ?? 0); ?>" aria-labelledby="<?php echo esc_attr($widget_id); ?>-title">
  ...
  <?php if(!empty($s['active_campaign']['goal'])): $campaign_metrics=$s['active_campaign']['_metrics'] ?? self::campaign_with_metrics($s['active_campaign'],$s['general']['currency'])['_metrics']; $progress=$campaign_metrics['percentage']; ?><div class="plugin-payments-campaign-progress" aria-label="<?php esc_attr_e('Campaign progress','plugin-ui-suite'); ?>"><span style="width:<?php echo esc_attr($progress); ?>%"></span></div><p class="plugin-payments-hint"><?php echo esc_html(self::money($campaign_metrics['raised'] ?? 0,$s['general']['currency']).' raised of '.self::money($s['active_campaign']['goal'],$s['general']['currency'])); ?></p><?php endif; ?>
  ...
  <?php endforeach; ?>
  <?php if(!empty($caps['fee_recovery']) && !empty($s['fee_recovery']['enabled']) && ($s['fee_recovery']['payer'] ?? 'rescue') === 'donor'): ?><label class="plugin-payments-fee-recovery"><input type="checkbox" data-payment-fee-recovery checked /> <span><?php echo esc_html($s['fee_recovery']['wording']); ?></span></label><p class="plugin-payments-total" data-payment-total aria-live="polite"></p><?php endif; ?>
  ...
  private static function render_payments_embed_page($settings) {
    $tag = self::SHORTCODE;
    $campaigns = array_keys((array)($settings['campaigns'] ?? []));
    $default_campaign = $campaigns ? $campaigns[0] : 'winter-appeal';
    echo '<section class="plugin-payments-section active" id="payments-embed"><h2>'.esc_html__('Donation Widget Embed','plugin-ui-suite').'</h2><p class="description">Use this page to copy the donation widget shortcode, configure common parameters and preview the saved widget before placing it on a public donation page.</p>';
    echo '<div class="plugin-suite-grid"><div class="plugin-suite-card"><h3>'.esc_html__('Shortcode','plugin-ui-suite').'</h3><p><input id="ssp-donation-shortcode" class="large-text code" readonly value="['.esc_attr($tag).']" /> <button type="button" class="button button-primary" data-ssp-copy-shortcode>'.esc_html__('Copy shortcode','plugin-ui-suite').'</button></p><p class="description">Paste this shortcode into a WordPress page, post, pattern, shortcode block or page-builder shortcode element.</p></div>';
    echo '<div class="plugin-suite-card"><h3>'.esc_html__('Parameters','plugin-ui-suite').'</h3><table class="widefat striped"><thead><tr><th>Parameter</th><th>Purpose</th><th>Example</th></tr></thead><tbody>';
    $rows = [['campaign','Load a saved campaign and its suggested amounts','campaign=&quot;'.esc_attr($default_campaign).'&quot;'],['default','Preselect one-off or monthly giving when the provider supports it','default=&quot;recurring&quot;']];
    foreach($rows as $row) echo '<tr><td><code>'.esc_html($row[0]).'</code></td><td>'.esc_html($row[1]).'</td><td><code>['.esc_html($tag).' '.$row[2].']</code></td></tr>';
    echo '</tbody></table><h4>'.esc_html__('Examples','plugin-ui-suite').'</h4><ul><li><code>['.esc_html($tag).']</code></li><li><code>['.esc_html($tag).' campaign=&quot;'.esc_html($default_campaign).'&quot;]</code></li><li><code>['.esc_html($tag).' default=&quot;recurring&quot;]</code></li></ul></div></div>';
    echo '<script>document.querySelector("[data-ssp-copy-shortcode]")?.addEventListener("click",function(){var input=document.getElementById("ssp-donation-shortcode");input.select();if(navigator.clipboard){navigator.clipboard.writeText(input.value);}this.textContent="Copied";});</script></section>';
  }

  private static function render_transactions_page($settings) {
    $rows = array_reverse((array)get_option(self::PAYMENT_EVENTS_KEY, []));
    echo '<section class="plugin-payments-section active" id="payments-transactions"><h2>'.esc_html__('Transactions','plugin-ui-suite').'</h2><p class="description">Completed checkouts, webhook updates, subscription events and provider references appear here after donors use the donation widget.</p>';
    echo '<table class="widefat striped"><thead><tr><th>Date</th><th>Checkout</th><th>Status</th><th>Provider</th><th>Campaign</th><th>Type</th><th>Amount</th></tr></thead><tbody>';
    if (!$rows) echo '<tr><td colspan="7"><strong>'.esc_html__('No transactions recorded yet.','plugin-ui-suite').'</strong><br/>'.esc_html__('Once a donor completes checkout or a payment webhook is received, transaction history will appear here with status, provider, campaign and amount details.','plugin-ui-suite').'</td></tr>';
    foreach($rows as $event){ $data=is_array($event['data']??null)?$event['data']:[]; $currency=strtoupper(sanitize_text_field($data['currency']??$event['currency']??($settings['general']['currency']??'GBP'))); $amount=self::dashboard_amount($data['amount']??$event['amount']??0,$currency); echo '<tr><td>'.esc_html($event['updated_at']??$event['created_at']??'').'</td><td><code>'.esc_html($event['id']??'').'</code></td><td>'.esc_html($event['status']??'').'</td><td>'.esc_html($data['provider']??'').'</td><td>'.esc_html($data['campaign']??'').'</td><td>'.esc_html($data['type']??'').'</td><td>'.esc_html(self::money($amount,$currency)).'</td></tr>'; }
    echo '</tbody></table></section>';
  }

  private static function render_reports_page($settings, $providers) {
    echo '<section class="plugin-payments-section active" id="payments-reports">';
    self::render_dashboard($settings, $providers);
    if (!get_option(self::PAYMENT_EVENTS_KEY, []) && !get_option(self::GIFT_AID_KEY, [])) echo '<div class="plugin-suite-card"><h3>'.esc_html__('Reports are ready for live donation data','plugin-ui-suite').'</h3><p>'.esc_html__('There is no payment data to chart yet. After checkout events are recorded, this page will show campaign progress, top providers, recurring income, success rate, refund rate and Gift Aid export activity.','plugin-ui-suite').'</p></div>';
    echo '</section>';
  }

  private static function render_payment_diagnostics_page($settings, $providers) {
    $gift_rows = get_option(self::GIFT_AID_KEY, []);
    echo '<section class="plugin-payments-section active" id="payments-diagnostics"><h2>'.esc_html__('Diagnostics','plugin-ui-suite').'</h2>';
    echo '<div class="plugin-suite-card"><h3>'.esc_html__('Payment provider checks','plugin-ui-suite').'</h3><table class="widefat striped"><thead><tr><th>Provider</th><th>Status</th><th>Message</th><th>Webhook URL</th></tr></thead><tbody>';
    foreach($providers as $id=>$provider){ $status=$provider->validate_configuration($settings['provider_settings'][$id]??[]); echo '<tr><td>'.esc_html($provider->get_name()).'</td><td>'.esc_html($status['connected']?__('Ready','plugin-ui-suite'):__('Needs configuration','plugin-ui-suite')).'</td><td>'.esc_html($status['message']).'</td><td><code>'.esc_html(rest_url('plugin-payments/v1/'.$id.'-webhook')).'</code></td></tr>'; }
    echo '</tbody></table></div><div class="plugin-suite-card"><h3>'.esc_html__('System information','plugin-ui-suite').'</h3><p><button type="button" class="button" data-ssp-copy-system>'.esc_html__('Copy System Information','plugin-ui-suite').'</button></p><textarea id="ssp-payment-system-info" class="large-text code" rows="7" readonly>'.esc_textarea(wp_json_encode(['plugin_version'=>PLUGIN_SUITE_VERSION,'wordpress'=>get_bloginfo('version'),'php'=>PHP_VERSION,'rest_base'=>rest_url('plugin-payments/v1/'),'active_provider'=>$settings['active_provider'],'providers'=>array_keys($providers),'payment_events'=>count((array)get_option(self::PAYMENT_EVENTS_KEY, [])),'webhook_events'=>count((array)get_option(self::WEBHOOK_EVENTS_KEY, [])),'gift_aid_declarations'=>count((array)$gift_rows)], JSON_PRETTY_PRINT)).'</textarea></div>';
    echo '<div class="plugin-suite-card"><h3>'.esc_html__('Gift Aid export','plugin-ui-suite').'</h3><p><a class="button" href="'.esc_url(wp_nonce_url(admin_url('admin-post.php?action=plugin_payments_export_gift_aid'),'plugin_payments_export_gift_aid')).'">'.esc_html__('Export Gift Aid CSV','plugin-ui-suite').'</a></p><p class="description">Exports all stored Gift Aid declarations. If there are no declarations yet, the CSV contains only headers.</p></div>';
    echo '<div class="plugin-suite-card"><h3>'.esc_html__('Recent logs','plugin-ui-suite').'</h3><pre>'.esc_html(wp_json_encode(['last_payment'=>array_slice((array)get_option(self::PAYMENT_EVENTS_KEY, []), -1),'last_webhook'=>array_slice((array)get_option(self::WEBHOOK_EVENTS_KEY, []), -1),'audit'=>array_slice((array)get_option(self::AUDIT_KEY, []), -10)], JSON_PRETTY_PRINT)).'</pre></div><script>document.querySelector("[data-ssp-copy-system]")?.addEventListener("click",function(){var t=document.getElementById("ssp-payment-system-info");t.select();if(navigator.clipboard){navigator.clipboard.writeText(t.value);}this.textContent="Copied";});</script></section>';
  }

  private static function render_registry_admin_page($section) {
    if (!class_exists('Plugin_UI_Suite_Registry')) return false;
    $section = sanitize_key($section ?: 'dashboard');
    $settings = self::settings();
    echo '<div class="wrap"><h1>'.esc_html__('Payments', 'plugin-ui-suite').' · '.esc_html(ucwords(str_replace('-',' ', $section))).'</h1>';
    echo '<nav class="plugin-suite-subnav plugin-payments-subnav">';
    foreach (Plugin_UI_Suite_Registry::navigation_items('payments') as $item) {
      $url = add_query_arg(['page'=>'plugin-ui-suite-payments-'.$item['slug']], admin_url('options-general.php'));
      printf('<a class="%s" href="%s">%s</a>', $section === $item['slug'] ? 'active' : '', esc_url($url), esc_html($item['label']));
    }
    echo '</nav>';
    if ($section === 'dashboard') { self::render_dashboard($settings, self::gateway_manager()->all()); echo '</div>'; return true; }
    if ($section === 'embed') { self::render_payments_embed_page($settings); echo '</div>'; return true; }
    if ($section === 'transactions') { self::render_transactions_page($settings); echo '</div>'; return true; }
    if ($section === 'reports') { self::render_reports_page($settings, self::gateway_manager()->all()); echo '</div>'; return true; }
    if ($section === 'diagnostics') { self::render_payment_diagnostics_page($settings, self::gateway_manager()->all()); echo '</div>'; return true; }
  ...
  private static function render_registry_payment_settings_section($section, $settings) {
    $fields = class_exists('Plugin_UI_Suite_Registry') ? Plugin_UI_Suite_Registry::settings_for_page($section) : [];
    $current_section = sanitize_key($_GET['payments_section'] ?? 'general');
    echo '<form method="post" action="options.php">'; settings_fields('plugin_payments_settings'); echo '<input type="hidden" name="_wp_http_referer" value="'.esc_url(add_query_arg(['page'=>'plugin-ui-suite','tab'=>'payments','payments_section'=>$current_section], admin_url('options-general.php'))).'" />';
    if (empty($fields)) { echo '<div class="plugin-suite-card"><p>'.esc_html__('This payments page is managed by the main Payments screen. Use the navigation above to open the matching production settings, embed guidance, reports or diagnostics view.', 'plugin-ui-suite').'</p></div>'; submit_button(__('Save payments settings', 'plugin-ui-suite')); echo '</form>'; return; }
    echo '<div class="plugin-suite-grid"><div class="plugin-suite-card"><table class="form-table" role="presentation">';
    foreach ($fields as $field) {
      $registry_id = str_replace('payments_', '', $field['id']);
      $parts = explode('_', $registry_id, 2); $group = $parts[0] === 'gift' ? 'gift_aid' : ($parts[0] === 'fee' ? 'fee_recovery' : ($parts[0] === 'appearance' ? 'appearance' : 'general'));
      $field_id = $field['field_id']; $value = $settings[$group][$field_id] ?? $field['default'] ?? '';
      $name = self::OPTION_KEY.'['.$group.']['.$field_id.']'; $html = '';
      if (($field['field_type'] ?? 'text') === 'checkbox') $html = '<label><input type="checkbox" name="'.esc_attr($name).'" value="1" '.checked($value,1,false).' /> '.esc_html($field['description'] ?? '').'</label>';
      elseif (($field['field_type'] ?? 'text') === 'select') { $html = '<select name="'.esc_attr($name).'">'; foreach ((array)($field['options'] ?? []) as $k=>$label) $html .= '<option value="'.esc_attr($k).'" '.selected($value,$k,false).'>'.esc_html($label).'</option>'; $html .= '</select>'; }
      elseif (($field['field_type'] ?? 'text') === 'color') $html = '<input type="color" name="'.esc_attr($name).'" value="'.esc_attr($value).'" />';
      else $html = '<input class="regular-text" type="text" name="'.esc_attr($name).'" value="'.esc_attr($value).'" />';
      echo '<tr><th scope="row">'.esc_html($field['label']).'</th><td>'.$html.'</td></tr>';
    }
    echo '</table></div></div>'; submit_button(__('Save payments settings', 'plugin-ui-suite')); echo '</form>';
  }

  private static function render_registry_provider_page($settings) {
    $current_section = sanitize_key($_GET['payments_section'] ?? 'general');
    echo '<form method="post" action="options.php">'; settings_fields('plugin_payments_settings'); echo '<input type="hidden" name="_wp_http_referer" value="'.esc_url(add_query_arg(['page'=>'plugin-ui-suite','tab'=>'payments','payments_section'=>$current_section], admin_url('options-general.php'))).'" />'; echo '<input type="hidden" name="'.esc_attr(self::OPTION_KEY).'[active_provider]" value="'.esc_attr($settings['active_provider']).'">';
    echo '<div class="plugin-suite-grid"><div class="plugin-suite-card"><h2>'.esc_html__('Choose provider','plugin-ui-suite').'</h2><select name="'.esc_attr(self::OPTION_KEY).'[active_provider]" data-registry-controller="active_provider">'; foreach (self::gateway_manager()->all() as $id=>$provider) echo '<option value="'.esc_attr($id).'" '.selected($settings['active_provider'],$id,false).'>'.esc_html($provider->get_name()).'</option>'; echo '</select></div>';
    foreach (Plugin_UI_Suite_Registry::integration_items('payment_provider') as $integration) { $id = str_replace('payment_', '', $integration['id']); if ($id !== $settings['active_provider']) continue; echo '<div class="plugin-suite-card"><h2>'.esc_html($integration['name']).'</h2><table class="form-table">'; foreach ((array)$integration['configuration_fields'] as $field=>$meta) { $type = ($meta['type'] ?? 'text') === 'password' ? 'password' : 'text'; $value = $settings['provider_settings'][$id][$field] ?? ''; echo '<tr><th>'.esc_html($meta['label'] ?? $field).'</th><td><input class="regular-text" type="'.esc_attr($type).'" name="'.esc_attr(self::OPTION_KEY).'[provider_settings]['.$id.']['.$field.']" value="'.esc_attr($value).'" /></td></tr>'; } echo '</table><p><strong>'.esc_html__('Supported features:', 'plugin-ui-suite').'</strong> '.esc_html(implode(', ', (array)$integration['supported_features'])).'</p></div>'; }
  ...
  public static function render_integration_provider_panel($integration_id) {
    ...
    echo '<tr><th>'.esc_html__('Test connection','plugin-ui-suite').'</th><td><a class="button" href="'.esc_url(add_query_arg(['page'=>'plugin-ui-suite','tab'=>'payments','payments_section'=>'diagnostics'], admin_url('options-general.php'))).'">'.esc_html__('Review diagnostics','plugin-ui-suite').'</a><p class="description">Saved credentials are validated immediately for required fields and key format. Use Payments → Diagnostics to review provider readiness, REST routes, webhook history and checkout audit records.</p></td></tr>';
  ...
  public static function render_admin_page($embedded=false) {
    ...
    $payments_tabs=['dashboard'=>'Dashboard','general'=>'Donation Settings','campaigns'=>'Campaigns','widget'=>'Donation Widget','gift-aid'=>'Gift Aid','fees'=>'Cover Fees','appearance'=>'Appearance','embed'=>'Embed','transactions'=>'Transactions','reports'=>'Reports','diagnostics'=>'Diagnostics','help'=>'Help'];
    ...
    echo '<section class="plugin-payments-section '.($current_section==='dashboard'?'active':'').'" id="payments-dashboard">'; self::render_dashboard($s, $providers); echo '</section>'; 
    ...
    if ($current_section === 'embed') { self::render_payments_embed_page($s); echo '<hr/><h2>'.esc_html__('Preview','plugin-ui-suite').'</h2><p class="description">'.esc_html__('This preview renders the saved donation widget exactly as WordPress will render it from the shortcode.','plugin-ui-suite').'</p><div class="ssp-payments-preview">'.self::render_shortcode().'</div>'; if (!$embedded) echo '</div>'; return; }
    if ($current_section === 'transactions') { self::render_transactions_page($s); if (!$embedded) echo '</div>'; return; }
    if ($current_section === 'reports') { self::render_reports_page($s, $providers); if (!$embedded) echo '</div>'; return; }
    if ($current_section === 'diagnostics') { self::render_payment_diagnostics_page($s, $providers); if (!$embedded) echo '</div>'; return; }
    ...
    echo '</section><section class="plugin-payments-section '.($current_section==='campaigns'?'active':'').'" id="payments-campaigns"><h2>'.esc_html__('Campaigns','plugin-ui-suite').'</h2><p class="description">'.esc_html__('Create reusable campaign overrides. Use the slug in the shortcode, for example [donation_widget campaign="winter"]. Suggested amount lines use: amount | impact description.','plugin-ui-suite').'</p><div id="ssp-campaigns">';
    ...
    foreach($campaign_rows as $i=>$campaign){
      ...
      foreach(['slug'=>'Short name (used in shortcode)','name'=>'Campaign Name','featured_image'=>'Image','end_date'=>'End date'] as $k=>$label) echo '<tr><th>'.esc_html($label).'</th><td><input class="regular-text" name="'.esc_attr($name).'[campaigns]['.(int)$i.']['.esc_attr($k).']" value="'.esc_attr($campaign[$k]??'').'"/></td></tr>';
      ...
      echo '<tr><th>'.esc_html__('Goal','plugin-ui-suite').'</th><td><input class="small-text" name="'.esc_attr($name).'[campaigns]['.(int)$i.'][goal]" value="'.esc_attr($campaign['goal']??'').'"/><p class="description">Amount raised is calculated automatically from successful donations assigned to this campaign.</p></td></tr><tr><th>'.esc_html__('Status','plugin-ui-suite').'</th><td><select name="'.esc_attr($name).'[campaigns]['.(int)$i.'][status]"><option value="active" '.selected($campaign['status']??'active','active',false).'>Active</option><option value="archived" '.selected($campaign['status']??'active','archived',false).'>Archived</option></select> <label><input type="checkbox" name="'.esc_attr($name).'[campaigns]['.(int)$i.'][default_campaign]" value="1" '.checked($campaign['default_campaign']??0,1,false).'/> '.esc_html__('Default Campaign','plugin-ui-suite').'</label></td></tr>';
      foreach(['primary'=>'Colour'] as $k=>$label) echo '<tr><th>'.esc_html($label).'</th><td><input type="color" name="'.esc_attr($name).'[campaigns]['.(int)$i.']['.esc_attr($k).']" value="'.esc_attr($campaign[$k]??'#ffffff').'"/></td></tr>';
      $metrics = !empty($campaign['slug']) ? self::campaign_with_metrics($campaign, $s['general']['currency'])['_metrics'] : ['raised'=>0,'goal'=>(float)($campaign['goal']??0),'remaining'=>(float)($campaign['goal']??0),'percentage'=>0,'donor_count'=>0,'recurring_monthly'=>0,'recent'=>[]]; echo '<tr><th>'.esc_html__('Campaign totals','plugin-ui-suite').'</th><td><strong>'.esc_html(self::money($metrics['raised'],$s['general']['currency'])).'</strong> raised · '.esc_html(self::money($metrics['remaining'],$s['general']['currency'])).' remaining · '.esc_html(round($metrics['percentage'])).'% complete · '.esc_html($metrics['donor_count']).' donors · '.esc_html(self::money($metrics['recurring_monthly'],$s['general']['currency'])).' monthly</td></tr>';
    ...
    echo '</section><section class="plugin-payments-section '.($current_section==='gift-aid'?'active':'').'" id="payments-gift-aid"><h2>'.esc_html__('Gift Aid','plugin-ui-suite').'</h2><table class="form-table" role="presentation"><tr><th>Enable</th><td><input type="checkbox" name="'.esc_attr($name).'[gift_aid][enabled]" value="1" '.checked($s['gift_aid']['enabled']??0,1,false).'/></td></tr><tr><th>Consent wording</th><td><input class="regular-text" name="'.esc_attr($name).'[gift_aid][wording]" value="'.esc_attr($s['gift_aid']['wording']??'').'"/></td></tr><tr><th>Declaration text</th><td><textarea class="large-text" rows="3" name="'.esc_attr($name).'[gift_aid][declaration]">'.esc_textarea($s['gift_aid']['declaration']??'').'</textarea></td></tr><tr><th>Behaviour</th><td><label><input type="checkbox" name="'.esc_attr($name).'[gift_aid][required]" value="1" '.checked($s['gift_aid']['required']??0,1,false).'/> Required</label> <label><input type="checkbox" name="'.esc_attr($name).'[gift_aid][store_declaration]" value="1" '.checked($s['gift_aid']['store_declaration']??0,1,false).'/> Store declaration</label></td></tr></table></section><section class="plugin-payments-section '.($current_section==='fees'?'active':'').'" id="payments-fees"><h2>'.esc_html__('Cover Processing Fees','plugin-ui-suite').'</h2><table class="form-table" role="presentation"><tr><th>Enable</th><td><input type="checkbox" name="'.esc_attr($name).'[fee_recovery][enabled]" value="1" '.checked($s['fee_recovery']['enabled']??0,1,false).'/></td></tr><tr><th>Who covers fees?</th><td><select name="'.esc_attr($name).'[fee_recovery][payer]"><option value="rescue" '.selected($s['fee_recovery']['payer']??'rescue','rescue',false).'>Rescue absorbs fees</option><option value="donor" '.selected($s['fee_recovery']['payer']??'rescue','donor',false).'>Donor covers fees</option></select><p class="description">Fees are calculated automatically from the active provider profile.</p></td></tr><tr><th>Advanced override</th><td><label><input type="checkbox" name="'.esc_attr($name).'[fee_recovery][override_defaults]" value="1" '.checked($s['fee_recovery']['override_defaults']??0,1,false).'/> Override provider defaults</label> <input class="small-text" placeholder="%" name="'.esc_attr($name).'[fee_recovery][percentage]" value="'.esc_attr($s['fee_recovery']['percentage']??'').'"/> <input class="small-text" placeholder="fixed" name="'.esc_attr($name).'[fee_recovery][fixed]" value="'.esc_attr($s['fee_recovery']['fixed']??'').'"/></td></tr><tr><th>Custom wording</th><td><input class="regular-text" name="'.esc_attr($name).'[fee_recovery][wording]" value="'.esc_attr($s['fee_recovery']['wording']??'').'"/></td></tr></table></section>';
    echo '<section class="plugin-payments-section '.($current_section==='appearance'?'active':'').'" id="payments-appearance"><h2>'.esc_html__('Appearance','plugin-ui-suite').'</h2><table class="form-table" role="presentation">';
