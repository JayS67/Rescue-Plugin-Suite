...
final class Plugin_Payments_Module {
  ...
  public static function register_registry_metadata() {
    ...
    foreach (['dashboard','general','providers','widget','campaigns','gift-aid','fees','appearance','transactions','reports','diagnostics','help'] as $i=>$section) Plugin_UI_Suite_Registry::register_navigation('payments_'.$section, ['context'=>'payments','module'=>'payments','slug'=>$section,'label'=>ucwords(str_replace('-',' ', $section)),'order'=>$i]);
  }
...
<section id="<?php echo esc_attr($widget_id); ?>" class="plugin-payments-widget plugin-payments-<?php echo esc_attr($s['appearance']['mode']); ?>" style="--ssp-primary:<?php echo esc_attr($s['appearance']['primary']); ?>;--ssp-on-primary:<?php echo esc_attr(self::readable_text_color($s['appearance']['primary'])); ?>;--ssp-bg:<?php echo esc_attr($s['appearance']['background']); ?>;--ssp-text:<?php echo esc_attr($s['appearance']['text']); ?>;--ssp-radius:<?php echo (int)$s['appearance']['radius']; ?>px" data-currency="<?php echo esc_attr($s['general']['currency']); ?>" data-default-type="<?php echo esc_attr($default); ?>" data-provider="<?php echo esc_attr($p?$p->get_id():''); ?>" data-campaign="<?php echo esc_attr($s['active_campaign']['slug'] ?? ''); ?>" data-fee-rate="<?php echo esc_attr(self::fee_profile($s['active_provider'],$s['general']['currency'])['rate'] ?? 0); ?>" data-fee-fixed="<?php echo esc_attr(self::fee_profile($s['active_provider'],$s['general']['currency'])['fixed'] ?? 0); ?>" aria-labelledby="<?php echo esc_attr($widget_id); ?>-title">
  ...
  private static function render_registry_payment_settings_section($section, $settings) {
    ...
    if (empty($fields)) { echo '<div class="plugin-suite-card"><p>'.esc_html__('This payments page is registry-controlled and has no editable fields yet.', 'plugin-ui-suite').'</p></div>'; submit_button(__('Save payments settings', 'plugin-ui-suite')); echo '</form>'; return; }
  ...
  public static function render_admin_page($embedded=false) {
    ...
    $payments_tabs=['dashboard'=>'Dashboard','general'=>'General','providers'=>'Providers','widget'=>'Donation Widget','campaigns'=>'Campaign Manager','gift-aid'=>'Gift Aid','fees'=>'Cover Processing Fees','appearance'=>'Appearance','transactions'=>'Transactions','reports'=>'Reports','diagnostics'=>'Diagnostics','help'=>'Help'];
    ...
    echo '<section class="plugin-payments-section '.($current_section==='general'?'active':'').'" id="payments-general"><h2>'.esc_html__('General','plugin-ui-suite').'</h2><table class="form-table" role="presentation">';
    ...
    echo '<section class="plugin-payments-section '.($current_section==='providers'?'active':'').'" id="payments-providers"><h2>'.esc_html__('Providers','plugin-ui-suite').'</h2><p>'.esc_html__('Enable payment providers, enter credentials, and review webhook and connection status. The selected provider controls the active checkout route.','plugin-ui-suite').'</p>';
    ...
    echo '</section><section class="plugin-payments-section '.($current_section==='campaigns'?'active':'').'" id="payments-campaigns"><h2>'.esc_html__('Campaign Manager','plugin-ui-suite').'</h2><p class="description">'.esc_html__('Create reusable campaign overrides. Use the slug in the shortcode, for example [donation_widget campaign="winter"]. Suggested amount lines use: amount | impact description.','plugin-ui-suite').'</p><div id="ssp-campaigns">';
